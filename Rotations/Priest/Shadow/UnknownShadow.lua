local rotationName = "UnknownShadow"
...
local function createOptions()
    ...
    local function generalOptions()
        ...
        br.ui:createSpinner(section, "Power Word: Shield (Body and Soul)", 5, 0, 100, 1, "Use Power Word: Shield (Body and Soul) after past seconds of moving.")
    ...
    local function damageOptions()
        ...
        br.ui:createCheckbox(section, "Mind Spike", "Use Mind Spike.")
...
actionList.PreCombat = function()
    if ui.checked("Pre-Pull Timer") and ui.pullTimer() <= ui.value("Pre-Pull Timer") then
        if unit.valid("target") then
			...
			if ui.checked("Shadowfiend") then
				if cast.shadowfiend("target") then ui.debug("Casting Shadowfiend [PreCombat]") return true end
			end
		end
    end
...
actionList.Extra = function()
    ...
    if br.IsMovingTime(ui.value("Angelic Feather")) then
        if ui.checked("Angelic Feather") and talent.angelicFeather and not buff.angelicFeather.exists("player") and not buff.soulshape.exists() then
            if cast.angelicFeather("player") then ui.debug("Casting Angelic Feather [Extra]") return true end
        end
    end
...
actionList.Defensive = function()
    if ui.useDefensive() then
		...
		if ui.checked("Power Word: Shield") and hp <= ui.value("Power Word: Shield") then
			if cast.powerWordShield("player") then ui.debug("Power Word: Shield [Defensive]") return true end
		end
    end
...
actionList.Healing = function()
    ...
    if ui.checked("Flash Heal") and not moving then
        if hp <= ui.value("Flash Heal") then
            if cast.flashHeal("player") then ui.debug("Casting Flash Heal [Healing]") return true end
        end
        if ui.checked("Heal Out of Combat") and not inCombat and hp <= ui.value("Heal Out of Combat") then
            if cast.flashHeal("player") then ui.debug("Casting Flash Heal OoC [Healing]") return true end
        end
    end
...
actionList.Damage = function()
    ...
    if ui.checked("Mind Spike") and not moving then
        if dotsUp and not debuff.vampiricTouch.exists(units.dyn40) then
            if cast.mindSpike(units.dyn40) then ui.debug("Casting Mind Spike [Damage]") return true end
        end
    end
...
--[[
    ...
    if ui.checked("Mind Spike") and not moving then
        if dotsUp and debuff.vampiricTouch.exists(units.dyn40) then
            if cast.mindSpike(units.dyn40) then ui.debug("Casting Mind Spike [Damage]") return true end
        end
    end
    if ui.checked("Mind Blast") and not moving then
